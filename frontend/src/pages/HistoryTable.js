...
// //       <h2>Stock History / Audit Trail</h2>
...
// //             <th style={thStyle}>Cartons Changed</th>
...
// //                 <td style={tdStyle}>{entry.product?.article || 'N/A'}</td>
// //                 <td style={tdStyle}>{entry.product?.color || 'N/A'}</td>
// //                 <td style={tdStyle}>{entry.product?.size || 'N/A'}</td>
// //                 <td style={tdStyle}>{entry.quantityChanged}</td>
// //                 <td style={tdStyle}>{entry.updatedByName || 'Unknown'}</td>
// //                 <td style={tdStyle}>{new Date(entry.timestamp).toLocaleString()}</td>
// //                 <td style={tdStyle}>{entry.note || '-'}</td>
// //                <td style={tdStyle}>
// //   {/* ✅ CORRECTED EDIT LINK LOGIC */}
// //   {entry.action === 'ADD' || entry.action === 'UPDATE' ? (
// //     <Link
// //       // Link sirf tabhi kaam karega jab salaryEntryId ho
// //       to={entry.salaryEntryId ? `/salary-entry/edit/${entry.salaryEntryId}` : '#'}
// //       style={{
// //         ...editBtnStyle,
// //         // Agar salaryEntryId nahi hai, to button ko disabled jaisa dikhao
// //         ...( !entry.salaryEntryId ? { opacity: 0.5, cursor: 'not-allowed' } : {}),
// //         textDecoration: 'none',
// //         display: 'inline-block'
// //       }}
// //       // onClick event se user ko rok do agar link kaam ka nahi hai
...
// //       ...deleteBtnStyle,
// //       marginLeft: '5px',
// //       ...((entry.quantityChanged < 0 && entry.action !== 'CHALLAN_OUT') ? { opacity: 0.5, cursor: 'not-allowed' } : {}),
// //     }}
// //     title={
// //       (entry.quantityChanged < 0 && entry.action !== 'CHALLAN_OUT')
// //         ? 'Delete disabled for negative carton changes (except Challan OUT)'
// //         : 'Delete entry'
// //     }
// //     onClick={() => {
// //       if (!(entry.quantityChanged < 0 && entry.action !== 'CHALLAN_OUT')) {
// //         handleDelete(entry._id);
// //       }
// //     }}
// //     disabled={entry.quantityChanged < 0 && entry.action !== 'CHALLAN_OUT'}
// //   >
// //     Delete
// //   </button>
                  
// //                   {/* Permanent Delete - Disabled only if CHALLAN_OUT */}
// //                   <button
// //                     style={{
// //                       ...permanentDeleteBtnStyle,
...
//   //const uniqueUpdatedBy = useMemo(() => [...new Set(history.map(entry => entry.updatedByName || 'Unknown'))].sort(), [history]);
...
//       //const entryText = `${entry.product?.article || ''} ${entry.product?.color || ''} ${entry.product?.size || ''} ${entry.note || ''} ${entry.updatedByName || ''}`.toLowerCase();
...
//      // const matchesUpdatedBy = selectedUpdatedBy ? (entry.updatedByName || 'Unknown') === selectedUpdatedBy : true;
...
//   {entry.partyName ? (
//     <>
//       Party: <Link to={`/party-summary?party=${encodeURIComponent(entry.partyName)}`}>{entry.partyName}</Link> | Inv: {entry.invoiceNo || '-'}
//     </>
//   ) : (entry.note || '-')}
// </td>
...
//           ))}
//         </select>
//         {/* New Link: Party Summary (next to Updated By dropdown) */}
//         <Link 
//           to="/party-summary"
//           style={{
//             padding: '8px 12px',
