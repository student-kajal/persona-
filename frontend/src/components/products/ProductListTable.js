...
const ProductListTable = ({ products, loading, title }) => {
  const [selected, setSelected] = useState([]);
  const [search, setSearch] = useState('');
  const [filter, setFilter] = useState({
    size: [],
  ...
  // 1) Central filtered list (search + all filters)
  const filteredList = useMemo(() => {
    ...
    return products.filter(p => {
      ...
      const matchesFilters =
        ...
        (filter.gender.length === 0 || filter.gender.includes(p.gender)) &&
        (filter.article.length === 0 || filter.article.includes(p.article)) &&
        (filter.stockType.length === 0 ||
          filter.stockType.includes(p.stockType)) &&
        (filter.pairPerCarton.length === 0 ||
          filter.pairPerCarton.includes(String(p.pairPerCarton))) &&
        (filter.mrp.length === 0 || filter.mrp.includes(String(p.mrp))) &&
        (filter.rate.length === 0 || filter.rate.includes(String(p.rate))) &&
        (filter.series.length === 0 || filter.series.includes(p.series));
  ...
  // 2) Grouped products from filtered list
  const groupedProducts = useMemo(() => {
    const groups = {};
  ...
  // 3) Dropdown options from filtered list (single pass per field)
  const uniqueSizes = useMemo(
    () => [...new Set(filteredList.map(p => p.size).filter(Boolean))],
    [filteredList]
  );
  const uniqueColors = useMemo(
    () => [...new Set(filteredList.map(p => p.color).filter(Boolean))],
    [filteredList]
  );
  const uniqueGenders = useMemo(
    () => [...new Set(filteredList.map(p => p.gender).filter(Boolean))],
    [filteredList]
  );
  const uniqueArticles = useMemo(
    () => [...new Set(filteredList.map(p => p.article).filter(Boolean))],
    [filteredList]
  );
  const uniqueStockTypes = useMemo(
    () => [...new Set(filteredList.map(p => p.stockType).filter(Boolean))],
    [filteredList]
  );
  const uniquePairPerCartons = useMemo(
    () => [...new Set(filteredList.map(p => p.pairPerCarton).filter(Boolean))],
    [filteredList]
  );
  const uniqueMRPs = useMemo(
    () => [...new Set(filteredList.map(p => p.mrp).filter(Boolean))],
    [filteredList]
  );
  const uniqueRates = useMemo(
    () => [...new Set(filteredList.map(p => p.rate).filter(Boolean))],
    [filteredList]
  );
  const uniqueSeries = useMemo(
    () => [...new Set(filteredList.map(p => p.series).filter(Boolean))],
    [filteredList]
  );

  // 4) Flattened filtered products
  const filteredProducts = useMemo(
    () => Object.values(groupedProducts).flatMap(group => group.variants),
    [groupedProducts]
  );
    ...
    for (const [groupName, articleGroups] of sortedCategories) {
      ...
      const articleBlocks = articles.map(articleGroup => {
        ...
        articleGroup.variants.forEach(v => {
          ...
          if (!colorMap[color]) colorMap[color] = {};
          colorMap[color][size] =
            (colorMap[color][size] || 0) + (v.cartons || 0);
        });
        ...
        return {
          ...
          imageId:
            ...
            articleImages[articleGroup.article]
              ? wb.addImage({
                  base64: `data:image/${articleImages[articleGroup.article].ext};base64,${articleImages[articleGroup.article].base64}`,
                  extension: articleImages[articleGroup.article].ext,
                })
          ...
          pageColumns.forEach((col, colIdx) => {
            if (colArticleIdx[colIdx] < col.length) {
              ...
              if (colorRow) {
                ...
                const emptyCols =
                  (matrixExportType === 'withImage' ? 3 : 2) +
                  columnSizes[colIdx].length;
                dataRow.push(...new Array(emptyCols).fill(''));
              }
            } else {
              const emptyCols =
  ...
  const handleGeneratePDF = async () => {
    ...
    if (selected.length > 0) {
      ...
      const zeroStockSelected = selectedProducts.filter(
        p =>
          (p.totalPairs || p.cartons * p.pairPerCarton) === 0
      );
      if (zeroStockSelected.length > 0) {
        alert(
          'Warning: आपने zero stock वाले products select किए हैं। कृपया valid stock वाले products select करें।'
        );
        return;
      }
      selectedProducts.forEach(p => {
        ...
        if (!selectedGroups[groupKey]) {
          ...
          if (group && group.variants) {
            ...
            if (
              imageProduct &&
              !selected.includes(imageProduct._id)
            ) {
              additionalImageIds.push(imageProduct._id);
            }
          }
    ...
    try {
      ...
      if (err.code === 'ECONNABORTED') {
        alert(
          '⏱️ PDF generation is taking longer than expected. Please try selecting fewer products or try again.'
        );
      } else {
        alert('❌ PDF download failed. Please try again.');
      }
  ...
  const renderTableRows = () => {
    ...
    groupEntries.sort((a, b) => {
      ...
      const groupInfoA = getVirtualGroup(
        groupA.variants[0]?.stockType,
        groupA.gender
      );
      const groupInfoB = getVirtualGroup(
        groupB.variants[0]?.stockType,
        groupB.gender
      );
      ...
      if ((groupA.variants[0]?.series || '') < (groupB.variants[0]?.series || ''))
        return -1;
      if ((groupA.variants[0]?.series || '') > (groupB.variants[0]?.series || ''))
        return 1;
        ...
        rows.push(
          <tr key={product._id}>
            ...
            {isFirstInGroup && (
              <td
                rowSpan={groupSize}
                className="text-center align-middle bg-light border-span"
              >
                <strong>{group.article}</strong>
              </td>
            )}
            {isFirstInGroup && (
              <td
                rowSpan={groupSize}
                className="text-center align-middle border-span"
              >
                {group.gender}
              </td>
            )}
  ...
  return (
    <div className="container-fluid py-2 dashboard-bg">
      ...
      {title && (
        <h2 className="mb-3 text-center text-primary">{title}</h2>
      )}
