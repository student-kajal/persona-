...
//   }
// });
// Route 2: Products with Images PDF (Catalogue format - OPTIMIZED)
router.get('/:id/products', async (req, res) => {
  try {
    if (!mongoose.Types.ObjectId.isValid(req.params.id)) {
      return res.status(400).json({
    ...
    // PDF setup
    res.setHeader('Content-Type', 'application/pdf');
    res.setHeader('Content-Disposition', `attachment; filename="catalogue-${challan.invoiceNo.replace('/', '-')}.pdf"`);
    ...
    for(let i=0;i<groups.length;i++){
      ...
      doc.rect(containerX,cTop,containerW,cH)
         .fillColor('#455A64').fill()
         .strokeColor('#d0e0f5').lineWidth(2).stroke();

      /* ✅ OPTIMIZED IMAGE RENDERING - From cache */
      if(g.image && imageCache[g.image]){
        try{
          ...
          console.warn(`Image render failed: ${g.article}`);
        }
      }

      /* table */
      const tTop=cTop+(g.image?250:30);
      let y=tTop,rowH=24,colW=120;
      const vars = g.variants.filter(v=>(v.cartons||0)>0);
      const sizes =[...new Set(vars.map(v=>v.size?.trim().toUpperCase()))]
                     .filter(Boolean).sort((a,b)=>a-b);
